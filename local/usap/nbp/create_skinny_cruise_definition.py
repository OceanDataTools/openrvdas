#! /usr/bin/env python3
"""Create a cruise definition where the loggers output only raw
records (UDP, file and cached data server), and rely on a "parser"
logger to read, parse and reinject parsed data into the cached data
server.

Still a bit wonky - parse errors show up at in the parser logger, not
in the raw logger whose message is unparseable.
"""
import logging
...
NET_WRITER_TEMPLATE="""
  %LOGGER%->net:
    ...
    - class: ComposedWriter     # Also parse to fields and send via
      kwargs:                   # websocket to CachedDataServer
        transforms:
    ...
    stderr_writers:          # Turn stderr into DASRecord and send via
    - class: ComposedWriter  # websocket to CachedDataServer
      kwargs:
        transforms:
        - class: ToDASRecordTransform
          kwargs:
...
FILE_NET_WRITER_TEMPLATE="""
  %LOGGER%->net/file:
    ...
    - class: ComposedWriter    # Write raw to UDP port %RAW_UDP%
      kwargs:
        transforms:
        - class: PrefixTransform
          kwargs:
        ...
        - class: UDPWriter
          kwargs:
            port: %RAW_UDP%
            interface: %UDP_INTERFACE%
    - class: ComposedWriter     # Write raw to cached dataserver
      kwargs:
        transforms:
        - class: PrefixTransform
          kwargs:
        ...
        - class: CachedDataWriter
          kwargs:
            data_server: localhost:%WEBSOCKET%
    stderr_writers:          # Turn stderr into DASRecord, send to
    - class: ComposedWriter  # CachedDataServer via websocket
      kwargs:
        transforms:
        - class: ToDASRecordTransform
          kwargs:
...
PARSER_TEMPLATE = """
  ...
  parser->on:         # Retrieve 'raw' data from CDS, parse, and put back in
    name: parser->on
    readers:
    ...
    - class: ComposedWriter  # CachedDataServer via websocket
      kwargs:
        ...
        - class: ToDASRecordTransform
          kwargs:
            field_name: 'stderr:logger:parser'
        writers:
        - class: CachedDataWriter
          kwargs:
            data_server: localhost:%WEBSOCKET%
...
TRUE_WIND_TEMPLATE = """
  ...
  true_winds->on:
    ...
    - class: CachedDataWriter
      kwargs:
        data_server: localhost:%WEBSOCKET%
    stderr_writers:          # Turn stderr into DASRecord, send to cached
    - class: ComposedWriter  # data server via websocket.
      kwargs:
        transforms:
        - class: ToDASRecordTransform
          kwargs:
...
###############################
def assemble_timeout_writer(reader_template):
  timeout_logger = fill_vars(reader_template, VARS)
...
output += """
    ...
    network_timeout: network_timeout->off
"""
...
output += """
    parser: parser->on
    true_winds: true_winds->on
    network_timeout: network_timeout->on
"""
...
output += fill_vars(PARSER_TEMPLATE, VARS)
output += fill_vars(TRUE_WIND_TEMPLATE, VARS)
output += assemble_timeout_writer(NET_TIMEOUT_TEMPLATE)
