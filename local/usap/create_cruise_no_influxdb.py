...
All modes (except 'off') also write to the cached data server.

Two derived data loggers are also included: true_wind and snapshot; these
are written to the cached data server , but not disk.
...
# Read raw records from UDP, parse them and distribute to CDS
NET_READER_TEMPLATE = """  ########
  net_reader->off:
    name: net_reader->off
