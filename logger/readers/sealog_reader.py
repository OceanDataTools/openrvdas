...
class SealogReader():
    ...
    def __init__(self, uri, client_wsid=None, subs=None, check_cert=False):
        ...
        # We won't initialize our websocket connection until the first read()
        # call. At that point we'll launch an async process in a separate
        # thread that will wait for data from the websocket and put it in a
        # queue that read() will pop from.
    ...
    def _start_websocket(self):
        ...
        def _event_to_das_record(event) -> DASRecord:
            ...
            fields = {
                ...
                'event_author': event.get('event_author'),
                'event_free_text': event.get('event_free_text'),
                **{
                    f"event_option_{option['event_option_name']}": option["event_option_value"]
                    for option in event.get("event_options", [])
                }
            }

            return DASRecord(
                data_id=self.client_wsid,
                message_type='sealog_event',
                timestamp=timestamp(event.get('ts')),
                fields=fields
            )
        ...
        async def _websocket_loop(self):
            ...
            while not self.quit_flag:
                try:
                    ...
                    async with websockets.connect(self.uri, ssl=ssl_context) as ws:  # type: ignore
                        ...
                        while not self.quit_flag:
                            ...
                            if msg_obj['type'] and msg_obj['type'] == 'ping':
                                await ws.send(json.dumps(self.ping))
                                continue
                    ...
                    logging.info(f'SealogReader BrokenPipeError connecting to {self.uri}')
                except AttributeError as e:
                    logging.warning(f'SealogReader websocket loop error: {e}')
                except websockets.exceptions.ConnectionClosed:  # type: ignore
                    logging.info('SealogReader lost websocket connection to '
