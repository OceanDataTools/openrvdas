...
class RecordParser:
    ...
    def __init__(self, record_format=None,
        ...
        # the devices and device types to compile them.
            ...
            for device_type, device_type_def in self.device_types.items():
                format = device_type_def.get('format')
                if format is None:
            ...
            if not metadata and metadata_interval is not None:
                for device, device_def in self.devices.items():  # e.g. s330
                    device_type_name = device_def.get('device_type')  # Seapath330
    ...
    def parse_record(self, record):
        ...
        # whitespace.
        field_string = parsed_record.get('field_string')
    ...
    def parse_for_data_id(self, data_id, field_string):
        ...
        device = self.devices.get(data_id)
        if not device:
            if not self.quiet:
        ...
        device_definition = self.device_types.get(device_type)
        if not device_definition:
            if not self.quiet:
                logging.error('No definition found for device_type "%s"', device_type)
            return failure_values

        compiled_format_patterns = device_definition.get('compiled_format')
        ...
        device_fields = device.get('fields')
        ...
        for field_name, value in parsed_fields.items():
            variable_name = device_fields.get(field_name)
