...
class FileWriter(Writer):
    ...
    def __init__(self,
        ...
        # Can't use split_interval or split_by_time if filebase and filename
        # are None
    ...
    def _load_header(self, header, header_file):
        ...
        if header:
            return (header.rstrip(self.delimiter) + self.delimiter
                    if self.delimiter else header)
        ...
        if header_file:
            try:
                with open(header_file, "r", encoding="utf-8") as hf:
                    return (
                        hf.read().strip().rstrip(self.delimiter)
                        + self.delimiter
                        if self.delimiter
                        else hf.read().strip()
                    )
                ...
                raise ValueError(
                    f"Error reading header_file {header_file}: {e}")
    ...
    def _validate_split_interval(self, split_interval):
        ...
        if not isinstance(split_interval, str):
            raise ValueError("split_interval must be a string like '1H' "
                             "or '30M'")
    ...
    def _validate_date_format(self, date_format):
        ...
        if not date_format:
            if unit == "H":
                return (DEFAULT_DATETIME_STR if even_days
                        else DEFAULT_DATETIME_STR + "T%H00")
            if unit == "M":
                # Concise f-string format
                return (f"{DEFAULT_DATETIME_STR}T%H"
                        f"{'%M' if needs_minute else '00'}")
        ...
        if not ({"%m", "%d"} <= found or "%j" in found):
            raise ValueError("date_format must include %m, %d (month, day) "
                             "or %j (day-of-year).")
    ...
    def _get_file_date_format(self):
        ...
        # if the data is being split by N hours
            ...
            timestamp_hour = (self.split_interval[0] *
                              math.floor(timestamp_raw.hour /
                                         self.split_interval[0]))
            timestamp_proc = timestamp_raw.replace(hour=timestamp_hour,
                                                   minute=0, second=0)
        ...
        # if the data is being split by N minutes
            ...
            timestamp_minute = (self.split_interval[0] *
                                math.floor(timestamp_raw.minute /
                                           self.split_interval[0]))
            timestamp_proc = timestamp_raw.replace(minute=timestamp_minute,
                                                   second=0)
            self.next_file_split = (timestamp_proc +
                                    timedelta(seconds=self
                                              .split_interval_in_seconds))
    ...
    def _set_file(self, filename):
        ...
        logging.info("opening %s with mode=%s and encoding=%s",
                     filename, self.mode, self.encoding)
        ...
        # if self.split_by_time or self.split_interval is not None:
            ...
            if new_file_date_format != self.file_date_format:
                ...
                if new_file_date_format.startswith('^'):
                    ...
                    self._set_file(self.filebase + new_file_date_format +
                                   self.suffix)
