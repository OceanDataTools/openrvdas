...
class TestModBusSerialReader(unittest.TestCase):
    ...
    def make_mock_client(self, responses_by_function):
        """Create a mock ModbusSerialClient with queued responses per function."""
    ...
    ###########################################################################
    def read_str(self, reader):
        """Helper to always decode read() output to list of strings."""
        result = reader.read()
        return [line.decode("utf-8") if isinstance(line, bytes) else line for line in result]
    ...
    def test_read_register_blocks(self, mock_client_cls):
        ...
        reader = ModBusSerialReader(
            registers="0:2,3:5,6:8",
            port="/dev/ttyUSB0",
            interval=0.01,
            encoding="utf-8"
        )
        result = self.read_str(reader)
        expected_values = [str(x) if x is not None else 'nan' for x in self.flatten_blocks(SAMPLE_REGISTERS)]
        expected = [f"slave 1: {' '.join(expected_values)}"]
    ...
    def test_read_with_timeout(self, mock_client_cls):
        ...
        reader = ModBusSerialReader(
            registers="0:2,3:5,6:8,9:11",
            port="/dev/ttyUSB0",
            interval=0.01,
            encoding="utf-8"
        )
        result = self.read_str(reader)
        expected_values = [str(x) if x is not None else 'nan' for x in self.flatten_blocks(SAMPLE_TIMEOUT_REGISTERS)]
        expected = [f"slave 1: {' '.join(expected_values)}"]
        self.assertEqual(expected, result)
    ...
    def test_unicode_handling(self, mock_client_cls):
        responses = [self.make_response(1, [0xFF, 0xFE, 0xABCD])]
    ...
    def test_poll_file_single_slave(self, mock_client_cls):
        yaml_cfg = {"polls": [{"slave": 1, "function": "holding_registers", "registers": "0:2"}]}
        responses = [self.make_response(1, [10, 20, 30])]

        mock_client_cls.return_value = self.make_mock_client({"read_holding_registers": responses})

        with tempfile.NamedTemporaryFile("w", delete=False) as f:
            yaml.dump(yaml_cfg, f)
            fname = f.name

        reader = ModBusSerialReader(registers=None, scan_file=fname, interval=0, encoding="utf-8")
        result = self.read_str(reader)
    ...
    def test_poll_file_multiple_slaves(self, mock_client_cls):
        yaml_cfg = {
            "polls": [
                {"slave": 1, "function": "holding_registers", "registers": "0:1"},
                {"slave": 2, "function": "holding_registers", "registers": "10:11"}
            ]
        }
        responses = [
        ...
        result = self.read_str(reader)
        expected = [
            f"slave 1: {' '.join([str(x) for x in [1, 2]])}",
    ...
    def test_poll_file_mixed_function_codes(self, mock_client_cls):
        yaml_cfg = {
            "polls": [
                {"slave": 1, "function": "holding_registers", "registers": "0:1"},
                {"slave": 1, "function": "input_registers", "registers": "5:6"}
            ]
        }
    ...
    def test_unknown_function_returns_nan(self, mock_client_cls):
        yaml_cfg = {"polls": [{"slave": 1, "function": "invalid_function", "registers": "0:2"}]}
        mock_client_cls.return_value = self.make_mock_client({})
