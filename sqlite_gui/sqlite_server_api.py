...
class SQLiteServerAPI(ServerAPI):
    ...
    def _get_connection(self):
        ...
        dbfile = os.path.join(ourpath, "openrvdas.sql")

        # See if database exists
        dburi = "".join(["file:", dbfile, "?mode=ro"])
        try:
            cx = sqlite3.connect(dburi, uri=True)
            ...
            # Some other error
            logging.error(" Database errro: ", err)
            raise err
        else:
            cx.close()
    ...
    def _sql_query(self, query, *args):
        ...
        try:
            ...
            logging.error("Database is empty (no tables)")
            logging.error("Please read SQLiteAPI.md")
            return None
        except sqlite3.Error as err:
            logging.error("Database error: %s", err)
    ...
    def _get_database_timestamp(self):
        ...
        try:
            ...
            logging.error("Unhandled database error: %s",  err)
            raise err
    ...
    def get_logger_config_name(self, logger_id, mode=None):
        ...
        if mode is None:
            ...
            if logger is None:
                raise ValueError("Logger id %s has no mode!" % logger_id)
        ...
        if mode_configs is None:
            raise ValueError("Requested mode %s is not defined" % mode)
