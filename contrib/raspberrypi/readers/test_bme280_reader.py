...
from contrib.raspberrypi.readers.bme280_reader import BME280Reader   # noqa: E402


################################################################################
################################################################################
class TestBME280Reader(unittest.TestCase):
    ############################
    def test_interval(self):
        iterations = 4
        reader = BME280Reader(interval=1)
        now = time.time()
    ...
    def test_conversions(self):
        reader = BME280Reader()
        f_reader = BME280Reader(temp_in_f=True)
        in_reader = BME280Reader(pressure_in_inches=True)
        record = reader.read()
